...
const HangoutTable = () => {
  ...
  const getHangoutById = async (id) => {
    try {
      const response = await axios.get(`http://localhost:4000/hangout/${id}`);
  ...
  const handleUpdate = async (id) => {
    ...
    try {
      await axios.put(`http://localhost:4000/hangout/${id}`, formData, {
        headers: {
  ...
  const deleteHangout = async (id) => {
    try {
      await axios.delete(`http://localhost:4000/hangout/${id}`);
      Swal.fire({
        icon: "success",
        title: "Deleted",
