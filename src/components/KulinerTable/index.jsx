...
const KulinerTable = () => {
  ...
  const getKulinerById = async (id) => {
    try {
      const response = await axios.get(`http://localhost:4000/kuliner/${id}`);
  ...
  const handleUpdate = async (id) => {
    ...
    try {
      await axios.put(`http://localhost:4000/kuliner/${id}`, formData, {
        headers: {
  ...
  const deleteKulinerData = async (id) => {
    try {
      await axios.delete(`http://localhost:4000/kuliner/${id}`);
      Swal.fire({
        icon: "success",
        title: "Deleted",
